...
@section('contents')
    <div class="d-flex align-items-center justify-content-between">
        ...
        <a href="{{ route('products.create') }}" class="btn btn-primary">Tambahkan Produk</a>
    </div>
    <hr />
                ...
                @if($product->count() > 0)
                    @foreach($product as $rs)
                        <tr>
                            ...
                            <td class="align-middle"><img src="{{ asset('/storage/images/'.$rs->images ) }}" height="40" alt=""></td>
                            <td class="align-middle">{{ $rs->title }}</td>
                            <td class="align-middle">{{ $rs->stock }}</td>
                            <td class="align-middle">{{ $rs->product_code }}</td>
                            <td class="align-middle">{{ $rs->description }}</td>
                            <td class="align-middle">{{ $rs->expired_date }}</td>
                            <td class="align-middle">{{ $rs->jenis_satuan }}</td>
